...
class SwarmAgent:
    def __init__(self, position, energy, hidden_size=4):
        ...
        self.energy = energy
        self.age = 0
        self.hidden_state = np.random.rand(hidden_size) * 0.1  # Initialize hidden state
        self.weights = np.random.rand(hidden_size + 2, 3) - 0.5  # Genetic weights for linear combinations
    ...
    def update(self, neighbors, energy_sources):
        ...
        if closest_energy_source is not None and distance_to_closest_food < 5.0:  # Assume 5 is the radius to consume food
            food_source = energy_sources[closest_energy_source]
            if food_source.energy > 0.1:
                energy_gained = min(food_source.energy, 0.1)  # Agents can consume up to 0.1 energy per update
        ...
        if speed > 1.0:  # Limit speed
            self.velocity /= speed
        self.position += self.velocity

        # Update hidden state
        self.hidden_state += np.tanh(self.hidden_state)  # Simple recurrent update
        self.hidden_state = np.clip(self.hidden_state, -1.0, 1.0)  # Keep hidden state bounded
    ...
    def reproduce(self):
        if self.energy > 1.5:
            self.energy /= 2.1
            new_weights = self.weights + (np.random.rand(*self.weights.shape) - 0.5) * 0.1  # Mutate weights
            new_hidden_state = np.random.rand(len(self.hidden_state)) * 0.1  # Randomize hidden state
...
def main():
    ...
    plt.ion()
    fig, ax = plt.subplots()
    fig.canvas.manager.set_window_title("Swarm Simulation Metrics")  # Optional: Set the title of the plot window
    time_steps = []
    population_sizes = []
    ...
    def update_plot():
        ...
        ax.legend()
        fig.canvas.draw_idle()  # Use draw_idle to avoid forcing a redraw
        plt.pause(0.001)  # Keep the UI responsive without taking focus
    ...
    def update_viewer():
        ...
        for i, agent in enumerate(agents):
            # Query neighbors using spatial tree
            neighbor_indices = tree.query_ball_point(agent.position, r=50.0)  # Neighbor radius of 50
            neighbors = [agents[j] for j in neighbor_indices if j != i]
            agent.update(neighbors, energy_sources)
            offspring = agent.reproduce()
            if offspring:
                new_agents.append(offspring[0])  # Add only the new agent, ignore the weights
        agents.extend(new_agents)
