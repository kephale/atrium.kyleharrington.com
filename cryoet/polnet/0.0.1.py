...
def generate_synthetic_data(
    ...
    """
    Generate synthetic cryo-ET data using Polnet with added debug logging.
    """
    ...
    typer.echo("Debug: Input parameters:")
    typer.echo(f"PROTEINS_LIST: {PROTEINS_LIST}")
    typer.echo(f"MB_PROTEINS_LIST: {MB_PROTEINS_LIST}")
    typer.echo(f"MEMBRANES_LIST: {MEMBRANES_LIST}")
    ...
    with typer.progressbar(DETECTOR_SNR, label="Processing SNR values") as snr_progress:
        for SNR in snr_progress:
            for ii in range(num_tomos_per_snr):
                ...
                # Generate features with debug logging
                try:
                    typer.echo(f"\nDebug: Generating features for {RUN_NAME}")
                    typer.echo("Debug: all_features2 parameters:")
                    typer.echo(f"NTOMOS: {NTOMOS}")
                    typer.echo(f"VOI_SHAPE: {VOI_SHAPE}")
                    typer.echo(f"permanent_dir: {permanent_dir}")
                    typer.echo(f"VOI_OFFS: {VOI_OFFS}")
                    typer.echo(f"voxel_size: {voxel_size}")
                    typer.echo(f"MMER_TRIES: {MMER_TRIES}")
                    typer.echo(f"PMER_TRIES: {PMER_TRIES}")
                    typer.echo(f"MEMBRANES_LIST: {MEMBRANES_LIST}")
                    typer.echo(f"Empty list: []")
                    typer.echo(f"PROTEINS_LIST: {PROTEINS_LIST}")
                    typer.echo(f"MB_PROTEINS_LIST: {MB_PROTEINS_LIST}")
                    typer.echo(f"SURF_DEC: {SURF_DEC}")
                    typer.echo(f"TILT_ANGS: {TILT_ANGS}")
                    typer.echo(f"SNR: {SNR}")
                    typer.echo(f"MALIGN_MIN: {MALIGN_MIN}")
                    typer.echo(f"MALIGN_MAX: {MALIGN_MAX}")
                    typer.echo(f"MALIGN_SIGMA: {MALIGN_SIGMA}")
                    ...
                    typer.echo(f"Debug: Exception type: {type(e)}")
                    typer.echo(f"Debug: Exception args: {e.args}")
                    continue
